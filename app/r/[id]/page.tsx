...
export async function generateMetadata({
  ...
  const who = meta?.name ? `${meta.name}님이` : "누군가";
  const title = `${who} 보낸 변화 영상 ✦ Morphoint`;
  const description =
    "시간에 따라 변하는 순간을 자연스러운 영상으로. 나도 무료로 만들어 보세요.";
...
export default async function SharePage({
  ...
  const who = meta.name ? `${meta.name}님이 보낸` : "누군가 보낸";
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col px-4 pb-10">
      ...
      <p className="mx-auto mt-1 max-w-xs text-center text-sm text-fg-soft">
        시간에 따라 변하는 순간을 자연스럽게 이어 만든 영상이에요.
      </p>

      <div className="card mt-5 overflow-hidden p-0">
        {isGif ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={src} alt={`${who} 변화 영상`} className="w-full" />
        ) : (
          <video
            src={src}
            poster={poster}
      ...
      <p className="mt-3 text-center text-xs text-fg-faint">
        사진 여러 장을 올리면 눈·코·입을 맞춰 자연스러운 영상으로 만들어 드려요.
        모든 처리는 내 기기 안에서.
      </p>
    </main>
  );
